"""
Temperature-dependent PDF fitting for SrFe2As2 phase transition study.
...
from diffpy.srfit.fitbase import FitResults, FitRecipe, initializeRecipe
...
REFINEMENT_TAGS = ["scale", "lattice_params", "adps", "atomic_positions", "delta2"]
R_MIN, R_MAX = 0.5, 50
...
def get_temperature_from_filename(filepath):
    """Extract temperature value from filename like 'SrFe2As2_150K.gr'"""
...
def fit_single_temperature(gr_file, cif_file, rmin, rmax):
...
def main():
    ...
    for gr in gr_files:
        for cif in cif_files:
            recipe, results = fit_single_temperature(gr, cif, R_MIN, R_MAX)
